let state = {
    'querySet': JSON.parse(localStorage.getItem("emp_array") || "[]"),
...
function buildTable(state) {
    ...
    state.page = JSON.parse(localStorage.getItem("empPageNo"));
...
function sortByColumnName(columnName){
    let emp_array = JSON.parse(localStorage.getItem("emp_array",) || "[]");
    ...
    state.querySet = emp_array;
    localStorage.setItem("emp_array", JSON.stringify(emp_array));
    //addDataToTable();
    buildTable(state);
...
function searchByIdOrName(){
    ...
    let found = false;
    if(String(searchValue).match(/^[A-Za-z0-9]+$/) && String(searchValue).startsWith("EM")){
        for(let obj of emp_arr){
            if(obj.id === searchValue.toUpperCase()){
                var tableBody = document.getElementById("tbody");
                tableBody.innerHTML = `<tr><td>${obj.id}</td><td>${obj.name}</td><td>${obj.age}</td>
                                        <td><input type="button" value="EDIT" onclick="editRecord(this)" class="edit-yellow">
        ...
        let temp = handleSearch(emp_arr, searchValue);

        var tableBody = document.getElementById("tbody");
        let dataHTML = '';
        for(let obj of temp){
            dataHTML += `<tr><td>${obj.id}</td><td>${obj.name}</td><td>${obj.age}</td>
                                        <td><input type="button" value="EDIT" onclick="editRecord(this)" class="edit-yellow">
                                        <input type="button" value="DELETE" onclick="deleteRecord(this)" class="delete-red"></td></tr>`;
        }
...
function deleteRecord(obj){
    ...
    if(confirmed){
        ...
        localStorage.setItem("emp_array", JSON.stringify(filtered_arr));
        state.querySet = JSON.parse(localStorage.getItem("emp_array") || "[]");
        buildTable(state);
    }
}
...
function editRecord(obj){
    ...
    localStorage.setItem("currObj", JSON.stringify({
        id : emp_id,
        name: emp_name,
        age: emp_age
    }));

    localStorage.setItem("fromEditRecord", JSON.stringify(true));
